...
abstract class BaseRepository
{
    ...
    public function approvalStatusChange(Model $object, $input)
    {
        $status = config("constants.approval_status.data.{$input['status']}", null);
        throw_if($status == null, BadRequestHttpException::class, 'Invalid status request.');
        $method = "mark$status";
        $this->model->$method($object, $input);
    }
     ...
     * @return \Illuminate\Database\Eloquent\Builder|\Illuminate\Database\Eloquent\Builder[]|\Illuminate\Database\Eloquent\Collection|Model|null
     */
    public function orderDetail($id, $columns = ['*'], $withTrashed = false, $slug = false)
    {
        $query = $this->model->newQuery();
