...
class RoleController extends AppBaseController
{
    ...
    public function ajaxRoles(Request $request)
    {
        ...
        $roles = $this->repository->all(['name' => ['exact' => $name], 'guard_name' => ['exact' => $platform]]);
        $guard = config("constants.platforms.$platform", null);
        $message = $roles->count() > 0 ? "Please enter different Title.<br/>The $name role is already exist, for the title $title and platform $guard." : '';
        return $this->sendResponse($name, $message);
    }
}
