...
@section('content')
    ...
    <div class="container">
        <h1 class="inner_page_heading">{{ trans('label.advance_review') }}</h1>
        <div class="row mb-50 jobseeker_dashboard review_main_wraper">
            @include('auth.job_seeker.profile.layout')
            <div class="col-md-8 col-lg-9 right_side">
                <div class="review-list mx-lg-0">
                    @if (count($advanceReviews) > 0)
                        @foreach ($advanceReviews as $review)
                            @if ($review->review_type == 2)
                                <div class="inner_item">
                                    <div class="box_wraper p-10">
                                        <div class="box_header d-flex align-items-center p-10 p-lg-15 position-relative">
                                            @if ($review->reviewToUser->hasRole('jobseeker'))
                                                @if (!empty($review->reviewToUser->seekerDetail) && $review->reviewToUser->seekerDetail->logo->count())
                                                    @include('vendor.image_upload.display', [
                                                        'document_type' => config(
                                                            'constants.document_type.image',
                                                            0),
                                                        'imageModel' =>
                                                            $review->reviewToUser->seekerDetail,
                                                        'class_li' => '',
                                                        'wrapper_class' => 'mr-15 profile_img',
                                                        'thumbnail' => true,
                                                    ])
                                                @else
                                            ...
                                            <div>
                                                <h3 class="mb-5">{{ $review->reviewToUser->first_name ?? '' }}
                                                    {{ $review->reviewToUser->last_name ?? '' }}</h3>
                                                <p class="mb-0">
                                                    {{ $review->created_at ? $review->created_at->format('M d, Y h:i A') : '' }}
                                                </p>

                                            </div>
                                            @include('components.advance_review_edit', [
                                                'entity' => $entity,
                                                'id' => $review->id,
                                            ])
                                        </div>
                                        <div class="card_body_wraper py-15 py-lg-20 px-12">
                                            <h4 class="section_title mb-15">Comments</h4>
                                            ...
                                            @if (!empty($review->audio))
                                                <div class="audio_file">
                                                    <audio controls>
                                                        <source src="{{ $review->audio->presigned_url }}"
                                                            type="audio/mpeg">
                                            ...
                                            @if (!empty($review->profilePic))
                                                <div class="img_video_section mb-20">
                                                    @include('vendor.image_upload.display', [
                                            ...
                                            <div class="description mb-25 pb-25">
                                                <p class="mb-0">{{ $review->review ?? '' }}</p>
                                            </div>
                                            <div class="respons_points_wraper points_wraper pb-15 mb-25">
                                                <h4 class="section_title mb-15">
                                                    {{ trans('label.responsibility') }}
                                                </h4>
                                                @if ($review->badge_strength)
                                                    <ul class="m-0 p-0 mb-8">
                                                        @foreach ($review->badge_strength as $strength)
                                                            <li>
                                                                <svg xmlns="http://www.w3.org/2000/svg"
                                                                    ...
                                                                    <g clip-path="url(#clip0_505_310)">
                                                                        <path
                                                                            ...
                                                                            stroke="#357DE8" stroke-width="1.5"
                                            ...
                                            <div class="weekness_points_wraper points_wraper">
                                                <h4 class="section_title mb-15">{{ trans('label.weakness') }}
                                                </h4>

                                                @if ($review->badge_weekness)
                                                    <ul class="m-0 p-0 mb-8">
                                                        @foreach ($review->badge_weekness as $weekness)
                                                            <li>
                                                                <svg xmlns="http://www.w3.org/2000/svg"
                                                                    width="18" height="18"
                                                                    viewBox="0 0 18 18" fill="none"
                        ...
                        @endforeach
                    @else
                        <div class="row border empty_review_listing h-100 mx-0">
                            <div
                                class="d-flex align-items-center justify-content-center flex-column col-12 py-50">
                                ...
                                <p class="mb-0 mt-20 text-black font-weight-bold no_found_text">No review
                                    available
                                    at
                                    the
                                    moment</p>
                    ...
                    <script>
                        $(document).ready(function() {
                            var dataItemsCount = $('.inner_item').length;

                            if (dataItemsCount === 2) {
                                $('body').addClass('only-two-reviews');
                            } else {
                                $('body').removeClass('only-two-reviews');
                            }

                            if (dataItemsCount === 0) {
                                $('body').addClass('empty-reviews');
                            } else {
                                $('body').removeClass('empty-reviews');
                            }
                        });
                    </script>
                </div>
