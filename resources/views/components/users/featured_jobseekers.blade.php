    ...
    <div class="inner_box">
        <div class="img_wraper">
            @if (!empty($user->seekerDetail) && $user->seekerDetail->logo->count())
                @include('vendor.image_upload.display', ['document_type' => config('constants.document_type.image', 0), 'imageModel' => $user->seekerDetail, 'class_li' => 'img-slick-slid', 'title' => $user->full_name, 'employerlogo' => 1])
            @else
                @include('vendor.image_upload.no_image', ['class_li' => 'img-slick-slid', 'title' => $user->full_name, 'employerlogo' => 1])
            @endif
        </div>
        <div class="content_wraper">
            <a href="{{ route('candidates.show', $user->slug) }}" class="d-block position-relative">
                <h3>{{ $user->full_name }}</h3>
                <p class="field">{{ $user->seekerDetail->title?? '' }}</p>
