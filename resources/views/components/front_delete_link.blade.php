...
<a class="{{$class??""}}" href="javascript:submitFormByaction('delete', '{{$entity['targetModel']}}_{{$id}}','{{$msg?? 'Do you want to delete this remark?' }}')" title="Delete {{$entity['singular']}}" data-toggle="tooltip" >
    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="16" viewBox="0 0 18 20" fill="none" class="">
...
{{-- <a class="{{$class??""}}" href="javascript:submitFormByaction('delete', '{{$entity['targetModel']}}_{{$id}}', '{!! trans('message.delete_msg',['module'=>$entity['singular']])!!}')" title="{{trans('label.delete')}} {{$entity['singular']}}">
