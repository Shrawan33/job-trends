...
@if(!empty($model->questionnaires) && $model->questionnaires->count() > 0)
    <a data-mode="show" data-modal-size="modal-lg"
        data-title="Answered by {{$model->user->full_name}}" data-model="question"
        data-url="{{ route('applicationTrackings.questionnaire',$model->id) }}"
        class="open-form  social_btn" style="cursor:pointer">
