<div class="list-item-view side_scroll px-30" id="load">
<p class="text-black mb-4 show_result_text">{!! __('label.showing') !!} {{$candidates->firstItem()??0}}-{{$candidates->lastItem()}} of {{$candidates->total()}} {!! __('label.result') !!}</p>
@php
...
<div class="row mt-30">
    ...
    @forelse($candidates->reverse() as $candidate)

    @include('candidates.card',['class' => "col-md-12", 'from' => 'candidate-list'])
...
{!! $candidates->render('vendor.pagination.custom',['prefix' => $prefix]) !!}
@push('page_scripts')
<script>
