...
<head>
    <title>Zoom Video Call TM</title>
    <meta charset="utf-8" />
...
<body>
            ...
            <div id="navbar" class="websdktest">
                <form class="navbar-form navbar-right" id="meeting_form">
                    <div class="form-group">
                        <input type="text" name="display_name" id="display_name" value="{{$meeting['data']['topic']??null}}" maxLength="100"
                            placeholder="Name" class="form-control" required>
                    </div>
                    <input type="hidden" name="meeting_join_url" id="meeting_join_url" value="{{ auth()->user()->roles->first()->name == 'employer' ? $meeting['data']['start_url'] : $meeting['data']['join_url']  }}"
                    >
                    <div class="form-group">
                        <input type="text" name="meeting_number" id="meeting_number" value="{{$meeting['data']['id']??null}}" maxLength="200" placeholder="Meeting Number" class="form-control" required>
                    </div>
                    <div class="form-group">
                        <input type="text" name="meeting_pwd" id="meeting_pwd" value="{{$meeting['data']['password']??null}}"
                            maxLength="32" placeholder="Meeting Password" class="form-control">
                    </div>
                    <div class="form-group">
                        <input type="text" name="meeting_email" id="meeting_email" value="{{$meeting['data']['host_email']??null}}"
                            maxLength="32" placeholder="Email option" class="form-control">
                    </div>

                    <div class="form-group">
                        <select id="meeting_role" class="sdk-select">
                            <option value=0 {{ auth()->user()->roles->first()->name == 'jobseeker' ? "selected" : ''  }}>Attendee</option>
                            <option value=1 {{auth()->user()->roles->first()->name == 'employer' ? "selected" : ''  }}>Host</option>
    ...
    <iframe width="100%" height="400" src="" id="show-zoom-video"></iframe>
    <script>

         var config = {
                routes: {
                    start: "{{ url('/meeting.html?') }}",
                    end: "{{ route('applyJobs.index') }}"
                }
            };
    </script>
    ...
    <script src="{{asset('js/meeting/tool.js')}}"></script>
    <script src="{{asset('js/meeting/vconsole.min.js')}}"></script>
    <script src="{{asset('js/meeting/index.js')}}"></script>
