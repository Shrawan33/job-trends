@extends('layouts.admin')

@section('content')
    <section class="content-header">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-6">
                    <h1>{!! __('label.categories') !!}</h1>
                </div>
                <div class="col-md-6 text-right">
                    {{-- @include('components.add_ajax_link', ['class' => 'btn btn-primary float-right', 'entity' => $entity, 'text' => trans('label.add_new')]) --}}
                    <a class="btn btn-primary float-right" href="{{ route($entity['url'].'.create') }}">{{__('label.add_new')}}</a>
                    <a class="btn btn-success open-form mr-5" data-mode="create" data-title="Import" data-model="category" data-url="{{ route('categories.import-file') }}" href="javascript:void(0)">{!! __('label.import') !!}</a>
                </div>
            </div>
        </div>
    </section>

    <div class="content px-3">
        <div class="clearfix"></div>
        @include('flash::message')
        <div class="clearfix"></div>

        <div class="card">
            <div class="card-body">
                @includeFirst([$entity['view'].'.table', 'components.admin.table'])
            </div>
        </div>
    </div>

@endsection

